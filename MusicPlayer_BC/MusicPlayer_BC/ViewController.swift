...
//        self.addViewsWithCode() //mainLoop는 동기적으로 처리를 하기 때문에 view들을 추가시키는 메소드를 먼저 구현하고 나서 그 view들을 동작시키는 함수를 입력하여야한다.
    ...
    func initializePlayer() {
        ...
        self.sliderProgress.maximumValue = Float(self.player.duration)
        self.sliderProgress.minimumValue = 0
        self.sliderProgress.value = Float(self.player.currentTime)
    }
    ...
    func updateTimeLabelText(time: TimeInterval) {
        ...
        self.labelTime.text = timeText
    }
    
    func makeAndFireTimer() {
        self.timer = Timer.scheduledTimer(withTimeInterval: 0.01, repeats: true, block: {[unowned self] (timer:Timer) in
            
            if self.sliderProgress.isTracking {return}
...
//        self.timer.fire()
    ...
    func addTimeLabel() {
        ...
        let centerX: NSLayoutConstraint
        centerX = timeLabel.centerXAnchor.constraint(equalTo: self.btnPlay.centerXAnchor)
        
        let top: NSLayoutConstraint
        top = timeLabel.topAnchor.constraint(equalTo: self.btnPlay.bottomAnchor, constant: 8)
        
        centerX.isActive = true
        top.isActive = true
        
        self.labelTime = timeLabel
    ...
    func addProgressSlider() {
        ...
        let centerX: NSLayoutConstraint
        centerX = slider.centerXAnchor.constraint(equalTo: self.labelTime.centerXAnchor)
        
        let top: NSLayoutConstraint
        top = slider.topAnchor.constraint(equalTo: self.labelTime.bottomAnchor, constant: 8)
